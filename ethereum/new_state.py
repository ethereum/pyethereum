...
class State():
    ...
    def is_CLEARING(self, at_fork_height=False):
        if at_fork_height: return self.block_number == self.config['CLEARING_FORK_BLKNUM']
        else: return self.block_number >= self.config['CLEARING_FORK_BLKNUM']
    ...
    def commit(self, allow_empties=False):
        for addr, acct in self.cache.items():
            if acct.touched:
                acct.commit()
                if acct.exists or allow_empties or (not self.is_CLEARING() and not acct.deleted):
