...
# integer_encode, integer_decode
ienc, idec = int_to_big_endian, big_endian_to_int
...
class WireProtocol():
    ...
    CAPABILITIES = 0x01 + 0x02  # node discovery + transaction relaying
    NODE_ID = None
    ...
    def rcv_packet(self, peer, packet):
        ...
        # check cmd
        if not len(data) or data[0] not in self.cmd_map:
            return self.send_Disconnect(peer, reason='Bad protocol')
    ...
    def send_packet(self, peer, data):
        ...
        packet = [ienc(self.SYNCHRONIZATION_TOKEN),
                  ienc(len(payload)),
                  payload]
