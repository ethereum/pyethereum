...
def test_abi_encode_single_ufixed():
    ...
    assert abi.encode_single(['ufixed', '128x128', []], 1.125) == (b'\x00'*15 + b'\x01\x20' + b'\x00'*15)
    assert abi.encode_single(['ufixed', '128x128', []], 2**127-1) == (b'\x7f' + b'\xff'*15 + b'\x00'*16)

def test_abi_encode_single_fixed():
    assert abi.encode_single(['fixed', '128x128', []], 1.125) == (b'\x00'*15 + b'\x01\x20' + b'\x00'*15)
    assert abi.encode_single(['fixed', '128x128', []], -1.125) == (b'\xff'*15 + b'\xfe' + b'\xe0' + b'\x00'*15)
...
def test_abi_decode_single_fixed():
    ...
    assert abi.decode_single(['fixed', '128x128', []], fixed_data) == (2**127-1)*1.0
